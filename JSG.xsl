...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <html lang="en">
            <head>
                ...
                <style>.prelim-req-table { margin-bottom: 1.5rem; } .prelim-req-table th, .prelim-req-table td { vertical-align: top; padding: 0.5rem; }</style>
            </head>
            <xsl:choose>
    ...
    <xsl:template match="description | procedure">
        ...
        <xsl:apply-templates select="levelledPara | para | figure | note | seqList | randomList | table | symbol"/>
    </xsl:template>
    <!-- CORRECTED `from` attributes in xsl:number -->
    ...
    <xsl:template match="levelledPara">
        <div class="levelled-para-container mb-3" id="{@id}">
            ...
            <div class="levelled-para-content">
                <xsl:apply-templates select="para | figure | note | seqList | randomList | table | levelledPara | symbol"/>
            </div>
        </div>
    </xsl:template>
    ...
    <xsl:template match="seqList">
        <xsl:variable name="listClass">
            <xsl:choose>
                <xsl:when test="ancestor::proceduralStep or ancestor::mainProcedure or ancestor::levelledPara or ancestor::note or ancestor::entry">s1000d-procedural-ol</xsl:when>
                <xsl:otherwise>s1000d-generic-ol</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template match="note[not(ancestor::safetyRqmts)]">
        <div class="alert alert-secondary s1000d-note ps-box" id="{@id}">
            ...
            <xsl:apply-templates select="para | seqList | randomList | node()[self::text() and normalize-space()]"/>
        </div>
    </xsl:template>
    ...
    <!-- CORRECTED `from` attributes and xs:integer cast -->
                ...
                <h5 class="s1000d-table-title text-center mt-2 mb-1" id="{generate-id(title)}">
                    <xsl:if test="$numbered-titles">
                        <span class="s1000d-table-title-prefix">Table
                            <xsl:number level="any" count="table" from="*[self::content or self::procedure or self::description or self::mainProcedure]" format="1"/>:
                        </span>
                    </xsl:if>
                    <xsl:apply-templates select="title" mode="inline"/>
                </h5>
                ...
                <h5 class="s1000d-figure-title" id="{generate-id(title)}">
                    <xsl:if test="$numbered-titles">
                        <span class="s1000d-figure-title-prefix">Figure
                            <xsl:number level="any" count="figure" from="*[self::content or self::procedure or self::description or self::mainProcedure]" format="1"/>:
                        </span>
                    </xsl:if>
                    <xsl:apply-templates select="title" mode="inline"/>
                </h5>
    ...
    <!-- CORRECTED `from` attributes and xsl:text usage -->
    ...
    <xsl:template match="symbol">
        <xsl:if test="@symbolType='toolIcon' or contains(lower-case(@id),'tool')">
