...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <html lang="en">
         <head>
            ...
            <style>
                    ...
                    .levelOneBar { margin-bottom: 1.5rem; border: 1px solid #eee; padding: 10px; }
                    .para0Indent { width: 40px; flex-shrink: 0; }
                    area { cursor: pointer; }
                    img.fig { border: 1px solid #ccc; }
                </style>
         </head>
         <body>
            <xsl:choose>
               ...
               <div class="text-center">
                  <xsl:for-each select="graphic">
                     ...
                     <div class="container-fluid m-0 pb-4">
                        ...
                        <xsl:if test="hotspot">
                           <map name="map_{@infoEntityIdent}">
                              <xsl:variable name="figno">
                                 <xsl:number count="figure" level="any"/>
                              </xsl:variable>
                              <xsl:for-each select="hotspot">
                                 <xsl:variable name="graIndex" select="count(parent::*/preceding-sibling::graphic) + 1"/>
                                 <xsl:variable name="hotIndex" select="count(preceding-sibling::hotspot)"/>
                                 <xsl:variable name="hotBase">
                                    <xsl:choose>
                                       <xsl:when test="@applicationStructureIdent and string-length(normalize-space(@applicationStructureIdent)) &gt; 0">
                                          <xsl:value-of select="@applicationStructureIdent"/>
                                       </xsl:when>
                                       <xsl:when test="@id and string-length(normalize-space(@id)) &gt; 0">
                                          <xsl:value-of select="@id"/>
                                       </xsl:when>
                                       <xsl:otherwise>
                                          <xsl:value-of select="concat('hot', format-number($hotIndex, '000'))"/>
                                       </xsl:otherwise>
                                    </xsl:choose>
                                 </xsl:variable>
                                 <xsl:variable name="areaId" select="concat('fig-', format-number(number($figno), '0'), '-gra-', $graIndex, '-hot-', $hotBase)"/>
                                 <area shape="rect">
                                    <xsl:attribute name="coords">
                                       <xsl:choose>
                                          <xsl:when test="@coords and string-length(normalize-space(@coords)) &gt; 0">
                                             <xsl:value-of select="@coords"/>
                                          </xsl:when>
                                          <xsl:otherwise>0,0,80,80</xsl:otherwise>
                                       </xsl:choose>
                                    </xsl:attribute>
                                    ...
                                    <xsl:attribute name="href">
                                       <xsl:value-of select="concat('#row-', $areaId)"/>
                                    </xsl:attribute>
                                    <xsl:attribute name="alt">
                                       <xsl:value-of select="@hotspotTitle"/>
                                    </xsl:attribute>
   ...
   <xsl:template name="parse-escaped-xml">
      <xsl:param name="text"/>
      <xsl:choose>
         <!-- Check if there is an escaped tag starting with &lt; -->
         <xsl:when test="contains($text, '&lt;') and contains($text, '&gt;')">
            <!-- 1. Output text BEFORE the tag -->
            ...
            <!-- Identify Tag Name (first word inside brackets) -->
            ...
            <!-- DISPATCHER: Handle tags based on name -->
            <xsl:choose>
               <!-- TYPE A: Self-closing tags (like symbol or graphic) -->
               <xsl:when test="$tagName = 'symbol' or $tagName = 'graphic'">
                  ...
                  <!-- Continue parsing text immediately after the > -->
                  <xsl:call-template name="parse-escaped-xml">
                     <xsl:with-param name="text" select="$remainingAfterTag"/>
                  </xsl:call-template>
               </xsl:when>
               <!-- TYPE B: Paired tags (internalRef, emphasis, etc.) -->
               <xsl:when test="$tagName = 'internalRef' or $tagName = 'emphasis' or $tagName = 'verbatimText'">
                  ...
                  <!-- Continue parsing text after the closing tag -->
                  <xsl:call-template name="parse-escaped-xml">
                     <xsl:with-param name="text" select="substring-after($text, $closingTag)"/>
                  </xsl:call-template>
               </xsl:when>
               <!-- FALLBACK: Not a recognized tag, output as raw text to avoid data loss -->
               <xsl:otherwise>
                  <xsl:text>&lt;</xsl:text>
                  <xsl:call-template name="parse-escaped-xml">
   ...
   <xsl:template match="entry">
      <td>
         <!-- FIX 2: Check if this cell contains a reference to a hotspot. 
                 If it does, give this cell an ID so the image can jump HERE. -->
         <xsl:if test="descendant::*[local-name()='internalRef']">
            <xsl:attribute name="id">
               <xsl:text>row-</xsl:text>
               <xsl:value-of select="descendant::*[local-name()='internalRef'][1]/@internalRefId"/>
            </xsl:attribute>
         </xsl:if>
   ...
   <!-- Modified Figure Template to support Hotspotting -->
            ...
            <div class="text-center font-italic mt-2 mb-1">
               ...
               <div class="text-center pb-4">
                  ...
                  <xsl:if test="graphic/hotspot">
                     <map name="map_{graphic/@infoEntityIdent}">
                        <xsl:variable name="figno">
                           <xsl:number count="figure" level="any"/>
                        </xsl:variable>
                  ...
                  <div class="icn small text-muted">
   ...
   <xsl:template match="graphic | symbol" mode="generate-image">
      <img type="image/png">
         ...
         <xsl:if test="local-name()='graphic' and hotspot">
            <xsl:attribute name="usemap">
               <xsl:value-of select="concat('#map_', @infoEntityIdent)"/>
            </xsl:attribute>
         </xsl:if>
