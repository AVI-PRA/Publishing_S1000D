...
<xsl:stylesheet
  ...
  version="1.0">
    ...
    <!-- *** ADD THESE LINES *** -->
    <xsl:variable name="mainIdentNode" select="/dmodule/identAndStatusSection/dmAddress/dmIdent | /pm/identAndStatusSection/pmAddress/pmIdent"/>
    <xsl:variable name="mainDmCodeNode" select="$mainIdentNode/dmCode | $mainIdentNode/pmCode"/>
    <xsl:variable name="mainLanguage" select="$mainDmCodeNode/@languageIsoCode"/>
    <xsl:variable name="mainCountry" select="$mainDmCodeNode/@countryIsoCode"/>
    <!-- *** END ADDED LINES *** -->
    ...
    <xsl:template match="sourceDmIdent">
        <!-- *** ADDED VARIABLES TO EXTRACT INFO *** -->
        <xsl:variable name="dmCodeNode" select="dmCode"/>
        <xsl:variable name="issueInfoNode" select="issueInfo"/>
        <xsl:variable name="langNode" select="language"/>
        <!-- *** MODIFIED: Use fallback if attribute missing *** -->
        <xsl:variable name="lang">
            <xsl:choose>
                <xsl:when test="$langNode/@languageIsoCode != ''">
                    <xsl:value-of select="$langNode/@languageIsoCode"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="country">
            <xsl:choose>
                <xsl:when test="$langNode/@countryIsoCode != ''">
                    <xsl:value-of select="$langNode/@countryIsoCode"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$mainCountry"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <!-- *** END MODIFIED *** -->
        <xsl:variable name="issno" select="$issueInfoNode/@issueNumber"/>
        <xsl:variable name="inwork" select="$issueInfoNode/@inWork"/>
        <!-- *** END ADDED VARIABLES *** -->
        ...
        <a>
            <xsl:attribute name="href">
                <xsl:call-template name="create-link">
                    ...
                    <xsl:with-param name="issue-num" select="$issno"/>
                    <xsl:with-param name="in-work-num" select="$inwork"/>
                </xsl:call-template>
            </xsl:attribute>
    ...
    <xsl:template match="dmRef">
        <!-- *** ADDED VARIABLES TO EXTRACT INFO *** -->
        <!-- <xsl:variable name="dmCodeNode" select="$identNode/dmCode"/> -->
        <xsl:variable name="issueInfoNode" select="dmRefIdent/issueInfo"/>
        <xsl:variable name="identNode" select="dmRefIdent"/>
        <xsl:variable name="issueInfoNode" select="$identNode/issueInfo"/>
        <xsl:variable name="langNode" select="$identNode/language"/>
        <!-- *** MODIFIED: Use fallback if attribute missing *** -->
        <xsl:variable name="lang">
            <xsl:choose>
                <xsl:when test="$langNode/@languageIsoCode != ''">
    ...
    <xsl:template name="create-link">
        ...
        <xsl:param name="issue-num"/>
        <xsl:param name="in-work-num"/>
        <xsl:text>csdb</xsl:text>
        <xsl:choose>
            <xsl:when test="$publication and $document">
                ...
                <xsl:text>-</xsl:text>
                <xsl:value-of select="$in-work-num"/>
                <xsl:text>_</xsl:text>
        ...
        <xsl:for-each select="document($assigns)">
            <xsl:text>&amp;</xsl:text>
        ...
        <xsl:if test="$non-applic">&amp;non-applic=
            <xsl:value-of select="$non-applic"/>
        </xsl:if>
        <xsl:if test="$units">&amp;units=
            <xsl:value-of select="$units"/>
        </xsl:if>
        <xsl:if test="$unit-format">&amp;unit-format=
            <xsl:value-of select="$unit-format"/>
        </xsl:if>
        <xsl:if test="$comments">&amp;comments=
            <xsl:value-of select="$comments"/>
        </xsl:if>
        <xsl:if test="$changes">&amp;changes=
