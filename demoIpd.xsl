...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html lang="en">
            <head>
                ...
                <style>
                    ...
                    .ipdTbl th, .ipdTbl td { vertical-align: middle; padding: 0.5rem; }
                    .ipdTbl .description-cell { text-align: left; }
    ...
    <xsl:template match="*[local-name()='identAndStatusSection']" mode="page-setup">
        ...
        <xsl:if test="$show-references-table and not(ancestor::*[local-name()='dmodule'][1]//*[local-name()='illustratedPartsCatalog'])">
            <div class="container-fluid">
    ...
    <xsl:template name="parse-escaped-refs">
        <xsl:param name="text"/>
        <!-- Define the start and end markers we are looking for -->
        <xsl:variable name="startTag" select="'&lt;internalRef'"/>
        <xsl:variable name="endTag" select="'&lt;/internalRef&gt;'"/>
        <xsl:choose>
            <!-- Case 1: An escaped tag is found in the current text block -->
            <xsl:when test="contains($text, $startTag) and contains($text, $endTag)">
                <!-- Output any plain text that comes before the escaped tag -->
                <xsl:value-of select="substring-before($text, $startTag)"/>
                <!-- Isolate the content of the tag itself (e.g., ' internalRefId="X">Text</internalRef>') -->
                <xsl:variable name="tag_and_after" select="substring-after($text, $startTag)"/>
                <xsl:variable name="tag_content" select="substring-before($tag_and_after, $endTag)"/>
                <!-- Isolate the text that comes after the closing tag -->
                <xsl:variable name="text_after" select="substring-after($tag_and_after, $endTag)"/>
                <!-- Extract the 'internalRefId' attribute value from the tag string -->
                <xsl:variable name="target-id">
                    <xsl:value-of select="substring-before(substring-after($tag_content, 'internalRefId=&quot;'), '&quot;')"/>
                </xsl:variable>
                <!-- Extract the display text from between the > and </internalRef> -->
                <xsl:variable name="display-text">
                    <xsl:value-of select="substring-after($tag_content, '&gt;')"/>
                </xsl:variable>
                <!-- Use the extracted ID to find the target node in the XML document -->
                <xsl:variable name="target" select="key('ids', $target-id)"/>
                <!-- Generate the HTML <a> tag, reusing the logic from the main internalRef template -->
                <a href="#{$target-id}">
                    <xsl:choose>
                        <!-- Re-implement the same numbering and text generation logic -->
                        ...
                        <!-- Fallback to using the display text found inside the escaped tag -->
                        <xsl:when test="normalize-space($display-text)">
                ...
                <!-- IMPORTANT: Make a recursive call to process the rest of the text -->
                <xsl:call-template name="parse-escaped-refs">
                    <xsl:with-param name="text" select="$text_after"/>
                </xsl:call-template>
            </xsl:when>
            <!-- Case 2: No escaped tag was found, so just output the text as is -->
            <xsl:otherwise>
    ...
    <!--
      Template: match="text()"
      Purpose: This is the entry point. It captures every text node in the document
               and sends it to our parser template.
    -->
