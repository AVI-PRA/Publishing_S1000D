...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html lang="en">
            <head>
                ...
                <style>
                    ...
                    /* Fix for list numbering with figures */
                    ol li { list-style-position: outside; }
                    ol li .figure-in-list { display: block; margin: 10px 0; }
                    /* Continuous list numbering across separate lists */
                    .levelOneBar, .levelTwoBar, .levelThreeBar, .levelFourBar, .levelFiveBar { counter-reset: continuous-list; }
                    .sequential-list-continuous { list-style: none !important; }
                    .sequential-list-continuous > li::before { 
                        counter-increment: continuous-list; 
                        content: counter(continuous-list) ". "; 
                        margin-left: -1.5em; 
                        margin-right: 0.5em; 
                        font-weight: inherit; 
                    }
    ...
    <xsl:template match="*[local-name()='sequentialList'] | *[local-name()='seqList']">
        <div class="d-flex">
            ...
            <div class="para0TextDescription flex-grow-1 pb-3">
                <ol>
                    <!-- Add continuous numbering class if there are other sequential lists or figures in the same levelledPara -->
                    <xsl:variable name="parentLevelledPara" select="ancestor::*[local-name()='levelledPara'][1]"/>
                    <xsl:variable name="hasMultipleListsOrFigures" 
                        select="$parentLevelledPara and (count($parentLevelledPara//*[local-name()='sequentialList' or local-name()='seqList']) &gt; 1 or count($parentLevelledPara/*[local-name()='figure']) &gt; 0)"/>
                    <xsl:if test="$hasMultipleListsOrFigures">
                        <xsl:attribute name="class">sequential-list-continuous</xsl:attribute>
                    </xsl:if>
    ...
    <xsl:template name="handle-paired-tag">
        ...
        <xsl:choose>
            <!-- CASE FOR dmRef in Escaped Text -->
            <xsl:when test="$tagName = 'dmRef'">
                ...
                <a href="DMC-{$dmc}.html">
                    <xsl:choose>
                        <xsl:when test="normalize-space($titleNode/*[local-name()='techName'])">
                            <xsl:value-of select="$titleNode/*[local-name()='techName']"/>
                            <xsl:if test="normalize-space($titleNode/*[local-name()='infoName'])">
                                <xsl:text> - </xsl:text>
                                <xsl:value-of select="$titleNode/*[local-name()='infoName']"/>
                            </xsl:if>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template match="figure">
        <xsl:choose>
            <!-- When figure is inside a list item, use simpler structure -->
            <xsl:when test="ancestor::*[local-name()='listItem']">
                <div class="figure-in-list" id="{@id}">
                    <xsl:if test="title">
            ...
            <!-- Normal figure rendering outside lists -->
            <xsl:otherwise>
    ...
    <xsl:template match="*[local-name()='dmRef']">
        ...
        <!-- Fetch external title logic -->
        <xsl:variable name="ref-xml-path" select="concat($csdb-path, 'DMC-', $ref-dmc, '.xml')"/>
        <xsl:variable name="referenced-doc" select="document($ref-xml-path)"/>
        <xsl:variable name="titleNode" select="$referenced-doc/*[local-name()='dmodule']/*[local-name()='identAndStatusSection']/*[local-name()='dmAddress']/*[local-name()='dmAddressItems']/*[local-name()='dmTitle']"/>
        <a href="{$ref-filename}">
            <xsl:choose>
                <xsl:when test="$titleNode/*[local-name()='techName']">
                    <xsl:value-of select="$titleNode/*[local-name()='techName']"/>
                    <xsl:if test="normalize-space($titleNode/*[local-name()='infoName'])">
                        <xsl:text> - </xsl:text>
                        <xsl:value-of select="$titleNode/*[local-name()='infoName']"/>
    ...
    <xsl:template match="*[local-name()='dmRef']" mode="refs">
        <tr>
            <xsl:call-template name="add-applic-attribute"/>
            <td>
                <!-- This column now outputs ONLY the DMC code -->
                <xsl:variable name="ref-dmc_raw">
                    <xsl:apply-templates select="*[local-name()='dmRefIdent']/*[local-name()='dmCode']" mode="text"/>
                </xsl:variable>
                <xsl:variable name="ref-dmc" select="normalize-space($ref-dmc_raw)"/>
            ...
            <td>
                <!-- This column handles the Title (Info Name) -->
                <xsl:variable name="ref-dmc_raw">
                    <xsl:apply-templates select="*[local-name()='dmRefIdent']/*[local-name()='dmCode']" mode="text"/>
                </xsl:variable>
                <xsl:variable name="ref-dmc" select="normalize-space($ref-dmc_raw)"/>
                <xsl:variable name="ref-xml-path" select="concat($csdb-path, 'DMC-', $ref-dmc, '.xml')"/>
                <xsl:variable name="referenced-doc" select="document($ref-xml-path)"/>
                <!-- Fetch infoName and techName for the Title column -->
                <xsl:variable name="titleNode" select="$referenced-doc/*[local-name()='dmodule']/*[local-name()='identAndStatusSection']/*[local-name()='dmAddress']/*[local-name()='dmAddressItems']/*[local-name()='dmTitle']"/>
                <xsl:variable name="tech-name" select="$titleNode/*[local-name()='techName']"/>
                <xsl:variable name="info-name" select="$titleNode/*[local-name()='infoName']"/>
                <xsl:choose>
                    <xsl:when test="normalize-space($tech-name)">
                        <xsl:value-of select="$tech-name"/>
                        <xsl:if test="normalize-space($info-name)">
                            <xsl:text> - </xsl:text>
