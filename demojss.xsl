...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- ################################################################## -->
    <!-- ### CORE CONTENT TEMPLATES WITH 3-LEVEL NUMBERING ### -->
    <!-- ################################################################## -->
    ...
    <xsl:template match="*[local-name()='levelledPara']">
        <xsl:variable name="depth" select="count(ancestor::*[local-name()='levelledPara'])"/>
        <div class="mb-3 p-3 border rounded lp-level-{$depth}" >
            <xsl:call-template name="add-applic-attribute"/>
            <div class="d-flex align-items-baseline mb-2">
                <div class="font-weight-bold mr-2">
                    <!-- CHANGE: Format updated to allow for three levels -->
                    <xsl:number level="multiple" count="*[local-name()='description' or local-name()='procedure'] | *[local-name()='levelledPara']" format="1.1.1"/>
                </div>
        ...
        <a href="#{$target-id}">
            <xsl:choose>
                ...
                <xsl:when test="local-name($target)='levelledPara'">
                    ...
                    <!-- CHANGE: Format updated to match the levelledPara template -->
                    <xsl:number level="multiple" count="*[local-name()='description' or local-name()='procedure'] | *[local-name()='levelledPara']" format="1.1.1"/>
                </xsl:when>
